...
public class GenreViewCreate {
    ...
    public void createGenreView() {
        ...
        try {
            mongoTemplate.getDb().getCollection("genre_view").drop();
        } catch (Exception ignored) {

        }

        mongoTemplate.getDb().createView("genre_view",    // view name
                                         "genre",         // source collection
                                         Arrays.asList(
                                                 Aggregates.addFields(new Field<>("createdUserId", "$created_user.$id"),
                                                                      new Field<>("updatedUserId",
                                                                                  "$updated_user.$id")),
                                                 ...
                                                 Aggregates.project(Projections.fields(
                                                         Projections.include("id", "name", "created_date",
